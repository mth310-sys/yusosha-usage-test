...
export function installGoldRushDebug({core,onChange=()=>{}}={}){
  ...
  if(!gt.__goldRushDebugInstalled){
    ...
    gt.__goldRushDebug={nextResult:null,lastResult:null,lastStocks:null,manualGames:0,policy:'ONE_RESULT_PER_G_NO_AUTO_PLUS_ONE_DUPLICATION'};
    ...
    gt.completeGoldRushGame=()=>{
      ...
      if(gt.goldRushGameCount>=GOLD_RUSH_PROFILE.initialGames&&!rollGoldRushContinuation(gt.rng)){
        ...
        if((gt.extraRemainingGames??0)>0){gt.state='EXTRA_BONUS_ACTIVE';gt.extraResult='ACTIVE_RESUMED_AFTER_GOLD_RUSH';gt.lastEvent=`GOLD_RUSH_DEBUG_${key}_${stocks}_STOCK_END_RESUME_EXTRA`;return gt.snapshot();}
        return gt.finishExtraToGuaranteedNextSet();
      }
      gt.lastEvent=`GOLD_RUSH_DEBUG_${key}_${stocks}_STOCK_CONTINUE`;
      return gt.snapshot();
    };
  }
