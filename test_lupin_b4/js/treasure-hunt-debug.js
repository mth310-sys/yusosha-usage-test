...
export function installTreasureHuntDebug({core,onChange=()=>{}}={}){
  ...
  if(!gt.__treasureHuntDebug){
    ...
    gt.startTreasureHuntForTest=(scenario='BRIDGE_JUMP',hold=null)=>{
      ...
      gt.__treasureHuntDebug={active:true,scenario,hold,result:'ACTIVE',destination:null,awardPoints:null,artStock:false,carryoverPoints:0,source:'DEBUG_MANUAL_TREASURE_HUNT'};
      gt.state='TREASURE_HUNT_DEBUG_ACTIVE';gt.lastEvent=`TREASURE_HUNT_DEBUG_START_${scenario}${hold?`_${hold}`:''}`;return gt.snapshot();
    };
    ...
    gt.resolveTreasureHuntForTest=({success,destination='TREASURE',awardPoints=null,artStock=false}={})=>{
      ...
      const guarantee=x.hold?getTreasureHuntHoldGuarantee(x.hold):null;const scenario=getTreasureHuntScenario(x.scenario);
      ...
      if(destination==='TREASURE_RUSH'){
        x.active=false;x.result='SUCCESS';x.destination='TREASURE_RUSH';x.artStock=Boolean(artStock);gt.state='ACTIVE_SET';gt.lastEvent='TREASURE_HUNT_DEBUG_SUCCESS_TREASURE_RUSH_READY_FOR_MANUAL_RUSH_START';if(artStock)gt.recordStockAdd?.(1,'TREASURE_HUNT_IMMORTAL_BOND_DEBUG_STOCK');return gt.snapshot();
      }
      const award=Number(awardPoints);const floor=guarantee?.minimumTreasurePoints??0;if(!Number.isFinite(award)||award<floor||award<0)return false;
      const threshold=applyTreasureAwardToGoldChanceThreshold(gt,award,{eventPrefix:'TREASURE_HUNT_DEBUG_SUCCESS'});if(!threshold)return false;
      x.active=false;x.result=threshold.reachedOneMillion?'SUCCESS_1M_GOLD_CHANCE_PENDING':'SUCCESS';x.destination='TREASURE';x.awardPoints=award;x.artStock=Boolean(artStock);x.carryoverPoints=threshold.carryoverPoints;
      if(artStock)gt.recordStockAdd?.(1,'TREASURE_HUNT_DEBUG_STOCK');
      if(!threshold.reachedOneMillion){gt.state='ACTIVE_SET';gt.lastEvent=`TREASURE_HUNT_DEBUG_SUCCESS_TREASURE_PLUS_${award}`;}
      return gt.snapshot();
    };
  }

  const gameLog=document.getElementById('log')?.closest('.panel');if(!gameLog)return {render:()=>{}};let panel=document.getElementById('treasureHuntDebugPanel');
  if(!panel){panel=document.createElement('section');panel.className='panel';panel.id='treasureHuntDebugPanel';panel.innerHTML=`<h2>TREASURE HUNT / STEP 6Z MANUAL</h2><p class="note">自然突入率・通常成功率・成功後の直上乗せ/RUSH振り分けは未回収。確認済みの演出・保証下限・不滅の絆→RUSHだけを手動検証する。直上乗せで100万T到達時は共通Treasure閾値処理からGOLD CHANCE待機へ接続する。</p><div class="panel-head"><select id="treasureHuntScenario"><option value="BRIDGE_JUMP">橋を飛び越えろ</option><option value="CUT_THROUGH_WARSHIP">軍艦を斬り進め</option><option value="SHOOT_DOWN_COMBAT_HELICOPTER">戦闘ヘリを撃破せよ</option><option value="IMMORTAL_BOND">不滅の絆</option></select><select id="treasureHuntHold"><option value="">HOLDなし</option><option value="FLAME_LUPIN">炎ルパン / 成功+20万以上</option><option value="FUJIKO">不二子 / 成功+30万以上</option><option value="TAMACHAN">玉ちゃん / 成功+100万以上</option></select><button id="treasureHuntStart" type="button">START HUNT</button></div><div class="panel-head"><input id="treasureHuntAward" type="number" min="0" step="50000" value="200000" inputmode="numeric"><button id="treasureHuntFail" type="button" disabled>FAIL → ART</button><button id="treasureHuntTreasure" type="button" disabled>SUCCESS → TREASURE</button><button id="treasureHuntRush" type="button" disabled>SUCCESS → RUSH READY</button></div><pre id="treasureHuntDebugState">NOT RUN</pre>`;gameLog.parentNode.insertBefore(panel,gameLog);}
