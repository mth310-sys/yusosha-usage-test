  ...
  if(!gt.__treasureHuntDebug){
    ...
    gt.resolveTreasureHuntForTest=({success,destination='TREASURE',awardPoints=null,artStock=false}={})=>{
      ...
      if(destination==='TREASURE_RUSH'){
        ...
        gt.__treasureRushPendingSource=`TREASURE_HUNT_${x.scenario}${x.hold?`_${x.hold}`:''}`;
        gt.state='TREASURE_RUSH_DURATION_PENDING';
        gt.lastEvent=`TREASURE_HUNT_DEBUG_SUCCESS_TREASURE_RUSH_DURATION_PENDING_${gt.__treasureRushPendingSource}`;
        return gt.snapshot();
      }
