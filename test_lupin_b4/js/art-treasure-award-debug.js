...
export function installArtTreasureAwardDebug({core,onChange=()=>{}}={}){
  ...
  const chanceEyeDebug=installArtChanceEyeTreasureRushDebug({core,onChange});
  if(!gt.__artTreasureAwardDebug){
    gt.__artTreasureAwardDebug=freshState();
    const originalSnapshot=gt.snapshot.bind(gt);
    gt.snapshot=()=>({...originalSnapshot(),artTreasureAwardDebug:{...gt.__artTreasureAwardDebug,oreNoNaWaProfile:ORE_NO_NA_WA_PROFILE,policy:'MANUAL_AMOUNT_RESOLUTION_ONLY_NO_SYNTHETIC_DISTRIBUTION'}});
    ...
    gt.processExactTreasureEvents=()=>{
      ...
      gt.pendingExactTreasureAward=TREASURE_AWARD_PROFILE.oreNoNaWaLupinRush.awardPoints;
      const threshold=applyTreasureAwardToGoldChanceThreshold(gt,TREASURE_AWARD_PROFILE.oreNoNaWaLupinRush.awardPoints,{eventPrefix:'ORE_NO_NA_WA'});
      if(!threshold)return false;
      gt.battleContinuationPct=100;
      gt.__artTreasureAwardDebug.lastOreNoNaWaCarryover=threshold.carryoverPoints;
      gt.__artTreasureAwardDebug.lastResult='ORE_NO_NA_WA_1M_GOLD_CHANCE_PENDING';
      return true;
    };
  ...
  const gameLog=document.getElementById('log')?.closest('.panel');if(!gameLog)return {render:()=>{chanceEyeDebug?.render();}};
  let panel=document.getElementById('artTreasureAwardDebugPanel');
  if(!panel){panel=document.createElement('section');panel.className='panel';panel.id='artTreasureAwardDebugPanel';panel.innerHTML=`<h2>ART TREASURE AWARD / STEP 6Z MANUAL</h2><p class="note">通常T揃いは10万〜100万T、異空間は1G最低5万Tまで確認済み。正確な振り分け表は未回収なので、発生済みイベントの金額だけ手動解決する。100万超過分は獲得契機を問わず共通carryoverへ保存し、EXTRA後の次セットで再適用する。</p><div class="panel-head"><input id="normalTreasureAward" type="number" min="100000" max="1000000" step="50000" value="100000" inputmode="numeric"><button id="normalTreasureAwardApply" type="button" disabled>RESOLVE NEXT NORMAL T</button></div><div class="panel-head"><input id="ikukanTreasureAward" type="number" min="50000" step="50000" value="50000" inputmode="numeric"><button id="ikukanTreasureAwardApply" type="button" disabled>RESOLVE NEXT IKUKAN G</button></div><div class="panel-head"><button id="treasureCarryoverExtraChain" type="button" disabled>TRIGGER SHARED 1M CARRYOVER EXTRA CHAIN</button></div><pre id="artTreasureAwardDebugState">NOT RUN</pre>`;gameLog.parentNode.insertBefore(panel,gameLog);}
  const normalInput=document.getElementById('normalTreasureAward'),normalApply=document.getElementById('normalTreasureAwardApply'),ikukanInput=document.getElementById('ikukanTreasureAward'),ikukanApply=document.getElementById('ikukanTreasureAwardApply'),chainApply=document.getElementById('treasureCarryoverExtraChain'),state=document.getElementById('artTreasureAwardDebugState');
  const render=()=>{const snap=gt.snapshot(),x=snap.artTreasureAwardDebug,threshold=snap.treasureThreshold??{};const canNormal=core.phase==='WAIT_BET'&&gt.state==='ACTIVE_SET'&&gt.pendingTreasureAwardEvents>0;const unresolvedIkukan=Math.max(0,(gt.ikukanGameCount||0)-x.ikukanResolvedGames);const canIkukan=core.phase==='WAIT_BET'&&(gt.state==='ACTIVE_SET'||gt.state==='IKUKAN_EXIT_PENDING_RETURN_STAGE_MODEL')&&unresolvedIkukan>0;const canChain=core.phase==='WAIT_BET'&&gt.state==='ACTIVE_SET'&&threshold.extraChainPending===true&&(Number(gt.treasurePoints)||0)>=1000000;if(normalApply)normalApply.disabled=!canNormal;if(ikukanApply)ikukanApply.disabled=!canIkukan;if(chainApply)chainApply.disabled=!canChain;if(state)state.textContent=`CORE STATE       ${gt.state}\nTREASURE         ${gt.treasurePoints}\nNORMAL PENDING   ${gt.pendingTreasureAwardEvents}\nNORMAL RESOLVED  ${x.normalResolved}\nNORMAL LAST      ${x.lastNormalAward??'---'}\nIKUKAN GENERATED ${gt.ikukanGameCount}\nIKUKAN RESOLVED  ${x.ikukanResolvedGames}\nIKUKAN PENDING   ${unresolvedIkukan}\nIKUKAN LAST      ${x.lastIkukanAward??'---'}\n俺の名 CARRY     ${x.lastOreNoNaWaCarryover??'---'}\nPENDING CARRY    ${threshold.pendingCarryoverPoints??0}\nLAST CARRY APPLY ${threshold.lastAppliedCarryoverPoints??0}\nEXTRA CHAIN      ${threshold.extraChainPending?'PENDING':'NO'}\nCARRY SOURCE     ${threshold.lastSource??'---'}\nLAST RESULT      ${x.lastResult}\nAUTO AMOUNT      DISABLED\nCHAIN ORDER      MANUAL / PRESENTATION ORDER UNRESOLVED`;chanceEyeDebug?.render();};
  ...
  chainApply?.addEventListener('click',()=>{gt.triggerTreasureCarryoverExtraChainForTest?.();render();onChange();});
