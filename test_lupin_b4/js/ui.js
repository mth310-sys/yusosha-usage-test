...
export function renderState(core,logger){
  const s=core.snapshot(), cz=s.normal.cz, rize=s.normal.rize, raiun=s.normal.raiun, lg=s.normal.legendGate, gt=s.goldenTime;
  const gtActive=gt?.state!=='IDLE';
  const set=(id,value)=>{const el=document.getElementById(id);if(el)el.textContent=value;};
  set('game',`#${String(s.gameNo).padStart(6,'0')}`); set('setting',String(s.setting)); set('phase',s.phase); set('mode',gtActive?'GOLDEN_TIME':s.normal.mode);
  ...
  set('raiunPoints',raiun?`${raiun.points}/${raiun.targetPoints}`:'---'); set('raiunState',raiun?.state??'---'); set('raiunHighLevel',raiun?.highLevel??'---'); set('raiunHighGames',raiun?.highRemainingGames==null?'---':`${raiun.highGameCount}/${raiun.highRemainingGames}`); set('raiunVariant',raiun?.variant??'---'); set('raiunModeGames',raiun?.variant?`${raiun.modeGameCount}/${raiun.modeRemainingGames??'∞'}`:'---'); set('raiunResult',raiun?.result??'---'); set('legendGateRate',raiun?.legendGateRate??'---'); set('legendGateHits',String(raiun?.legendGateHits??0)); set('legendGateGame',raiun?.lastLegendGateGame==null?'---':`${raiun.lastLegendGateGame}G`);
  set('lgState',lg?.state??'---'); set('lgMedals',lg?String(lg.medals):'---'); set('lgMinStocks',lg?.minGtStocks==null?'---':`${lg.minGtStocks}+`); set('lgSetRange',lg?.expectedSetRange??'---');
  set('gtState',gt?.state??'IDLE'); set('gtSet',gtActive?String(gt.setNo):'---'); set('gtGame',gtActive?`${gt.gameInSet}/${gt.profile.setGamesApprox}`:'---'); set('gtRemain',gt?.remainingGames==null?'---':String(gt.remainingGames)); set('gtStocks',String(gt?.guaranteedStocks??0)); set('gtSource',gt?.entrySource??'---');
  renderHolds(s.normal.holdQueue);
  set('credit',String(s.credit)); set('bet',String(s.bet)); set('payout',String(s.payout)); set('role',s.role);

  const gtCanPlay=!gtActive||gt.state==='ACTIVE_SET';
  document.getElementById('betButton').disabled=s.phase!=='WAIT_BET'||!core.creditSystem.canBet()||!gtCanPlay;
  document.getElementById('leverButton').disabled=s.phase!=='WAIT_LEVER';
  document.getElementById('settingSelect').disabled=s.phase!=='WAIT_BET'||gtActive;
  document.getElementById('wantedSeek').disabled=s.phase!=='WAIT_BET'||s.normal.mode!=='NORMAL'||gtActive;
  document.getElementById('holdInject').disabled=s.phase!=='WAIT_BET'||s.normal.mode!=='WANTED_CHANCE'||gtActive; document.getElementById('holdTypeSelect').disabled=s.normal.mode!=='WANTED_CHANCE'||gtActive;
  const czReady=s.phase==='WAIT_BET'&&!gtActive&&['DOROBO_ZONE','FUJIKO_ZONE'].includes(s.normal.mode)&&!!cz; document.getElementById('czSuccess').disabled=!czReady; document.getElementById('czFail').disabled=!czReady;
  const normalReady=s.phase==='WAIT_BET'&&!gtActive&&s.normal.mode==='NORMAL'; document.getElementById('rizeEnter').disabled=!normalReady; document.getElementById('shinRizeEnter').disabled=!normalReady;
  const rizeReady=s.phase==='WAIT_BET'&&!gtActive&&s.normal.mode==='RIZE_ZONE'&&!!rize; document.getElementById('rizeBackgroundSelect').disabled=!rizeReady; document.getElementById('rizeBackgroundSet').disabled=!rizeReady; document.getElementById('rizeSuccess').disabled=!rizeReady; document.getElementById('rizeFail').disabled=!rizeReady;
  ...
  const raiunReady=s.phase==='WAIT_BET'&&!gtActive&&s.normal.mode==='RAIUN_MODE'; document.getElementById('raiunSuccess').disabled=!raiunReady; document.getElementById('raiunFail').disabled=!raiunReady||raiun?.variant==='SHIN_RAIUN';
  document.getElementById('legendGateEnter').disabled=!normalReady;
  const lgReady=s.phase==='WAIT_BET'&&!gtActive&&s.normal.mode==='LEGEND_GATE'&&!!lg; document.getElementById('legendGateMedalSelect').disabled=!lgReady; document.getElementById('legendGateMedalSet').disabled=!lgReady;
  const exactGtPending=['GOLDEN_TIME','GOLDEN_TIME_STOCKS'].includes(s.normal.pendingReward?.type); document.getElementById('goldenTimePendingStart').disabled=s.phase!=='WAIT_BET'||gtActive||!exactGtPending;
  document.getElementById('goldenTimeEnter').disabled=s.phase!=='WAIT_BET'||gtActive;
  document.getElementById('goldenTimeStockAdd').disabled=s.phase!=='WAIT_BET'||!gtActive;
