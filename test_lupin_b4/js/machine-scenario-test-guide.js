...
let scenario1=null,scenario2=null;
...
export function renderMachineScenarioTestGuide(core){const p=ensurePanel();if(!p)return;if(core&&!scenario1)scenario1=installNormalLbScenarioDebug({core,onChange:()=>{}});if(core&&!scenario2)scenario2=installNormalGtScenarioDebug({core,onChange:()=>{}});scenario1?.render();scenario2?.render();const coverage=document.getElementById('machineScenarioPanelState')?.textContent??'';const lines=STEPS.map(([name,guide],i)=>{const escaped=name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const m=coverage.match(new RegExp(`${escaped}\\s+(PASS|FAIL|NOT RUN)`));const state=m?.[1]??'NOT RUN';return `${String(i+1).padStart(2,'0')}. ${name.padEnd(24,' ')} ${state}\n    ${guide}`;});const next=STEPS.find(([name])=>!new RegExp(`${name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\s+PASS`).test(coverage));document.getElementById('machineScenarioGuideState').textContent=[`NEXT             ${next?.[0]??'ALL 7 COMPLETE'}`,...lines].join('\n');}
