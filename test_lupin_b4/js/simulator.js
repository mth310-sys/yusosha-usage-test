...
import { RAIUN_POINT_MODEL, drawInitialRaiunPoints, rollRaiunPointAdd, drawRaiunPointAdd } from './raiun-point-model.js?v=step5d';
  ...
  for (let cycle=0; cycle<cycles; cycle++) {
    ...
    while (points < 100) {
      ...
      if (rollRaiunPointAdd(rng)) {
        ...
        totalAdds+=1; totalAddedPoints+=add;
      }
    }
...
export function formatRaiunCycleReport(report) {
  ...
  return [
    ...
    'NOTE: RAW published aggregates are preserved; gameplay uses the isolated locked CALIBRATED effective rate.'
  ].join('\n');
}
