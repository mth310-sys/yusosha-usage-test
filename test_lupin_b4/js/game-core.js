...
import { NormalSystem } from './normal.js?v=step6t';
import { GoldenTimeSystem } from './golden-time.js?v=step6t';
import { RevengeChanceSystem } from './revenge-chance.js?v=step6t';
import { LupinBonusSystem } from './lupin-bonus.js?v=step6t';
import { LCD_CHANCE_PROFILE, rollLcdChance } from './lcd-chance-profile.js?v=step6t';

export class GameCore {
  constructor({setting=1, seed=Date.now()} = {}) { this.setting=Number(setting); this.profile=getSettingProfile(this.setting); this.rng=new RNG(seed); this.creditSystem=new CreditSystem(MACHINE.initialCredit,MACHINE.betPerGame); this.reels=new ReelController(this.rng); this.normal=new NormalSystem(this.rng,this.setting); this.goldenTime=new GoldenTimeSystem(this.rng,this.setting); this.revenge=new RevengeChanceSystem(); this.lupinBonus=new LupinBonusSystem(this.rng); this.lcdChance={last:null,totalHits:0,weakHits:0,middleHits:0,strongHits:0,profile:LCD_CHANCE_PROFILE}; this.gameNo=0; this.phase='WAIT_BET'; this.lastRole=null; this.pendingRole=null; this.creditBeforeGame=null; }
  ...
  processWantedLcdChance(){if(this.normal.mode!=='WANTED_CHANCE'||!this.normal.holdQueue)return null;const hit=rollLcdChance('WANTED_CHANCE',this.rng);if(!hit)return null;this.normal.holdQueue.injectNext(hit.holdType);this.lcdChance.totalHits+=1;if(hit.key==='WEAK_BLUE')this.lcdChance.weakHits+=1;else if(hit.key==='MIDDLE_RED')this.lcdChance.middleHits+=1;else if(hit.key==='STRONG_7')this.lcdChance.strongHits+=1;this.lcdChance.last={...hit,gameNo:this.gameNo};return this.lcdChance.last;}
  stopReel(index){if(this.phase!=='SPINNING')return null;const symbol=this.reels.stop(index);if(symbol==null)return null;if(!this.reels.allStopped)return{complete:false,symbol};this.phase='RESULT';this.creditSystem.settle(this.pendingRole);let normal=this.normal.snapshot(),gt=this.goldenTime.snapshot(),revenge=this.revenge.snapshot(),lupinBonus=this.lupinBonus.snapshot(),mode,event;
    if(this.revenge.state==='ACTIVE'){
      ...
      if(lupinBonus.state==='SUCCESS_ART_PENDING_GT'){
        this.goldenTime.reset();this.goldenTime.start({guaranteedStocks:0,source:'LUPIN_BONUS_WIN_STEP6T'});gt=this.goldenTime.snapshot();this.lupinBonus.reset();lupinBonus=this.lupinBonus.snapshot();mode='GOLDEN_TIME';event='LUPIN_BONUS_WIN_GOLDEN_TIME_AUTO_START';
      ...
      normal=this.normal.snapshot();gt=this.goldenTime.snapshot();mode=normal.mode;event=lcdHit?`LCD_CHANCE_${lcdHit.key}_HOLD_${lcdHit.holdType}_AUTO`:normal.lastEvent;
    }
  ...
  lcdChanceSnapshot(){return{last:this.lcdChance.last?{...this.lcdChance.last}:null,totalHits:this.lcdChance.totalHits,weakHits:this.lcdChance.weakHits,middleHits:this.lcdChance.middleHits,strongHits:this.lcdChance.strongHits,profile:this.lcdChance.profile};}
  ...
  startGoldenTimeFromPending(){if(!this.baseNormalReady())return false;const p=this.normal.pendingReward;if(!p||!['GOLDEN_TIME','GOLDEN_TIME_STOCKS'].includes(p.type))return false;const stocks=p.type==='GOLDEN_TIME_STOCKS'?(Number(p.minStocks)||0):0;const ok=this.goldenTime.start({guaranteedStocks:stocks,source:`PENDING_${p.source}`});if(ok){p.status='ROUTED_TO_GOLDEN_TIME_STEP6T';}return ok;}
  ...
  forceLupinBonusEarlyArtForTest(trigger='BATTLE_WIN'){if(this.phase!=='WAIT_BET'||!this.lupinBonus.forceVerifiedEarlyBattleWinForTest(trigger))return false;this.goldenTime.reset();this.goldenTime.start({guaranteedStocks:0,source:`LUPIN_BONUS_EARLY_${trigger}_STEP6T`});this.lupinBonus.reset();return true;}
