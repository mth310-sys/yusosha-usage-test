...
import { NormalSystem } from './normal.js?v=step6v';
import { GoldenTimeSystem } from './golden-time.js?v=step6v';
import { RevengeChanceSystem } from './revenge-chance.js?v=step6v';
import { LupinBonusSystem } from './lupin-bonus.js?v=step6v';
import { LCD_CHANCE_PROFILE, rollLcdChance } from './lcd-chance-profile.js?v=step6v';

export class GameCore {
  constructor({setting=1, seed=Date.now()} = {}) { this.setting=Number(setting); this.profile=getSettingProfile(this.setting); this.rng=new RNG(seed); this.creditSystem=new CreditSystem(MACHINE.initialCredit,MACHINE.betPerGame); this.reels=new ReelController(this.rng); this.normal=new NormalSystem(this.rng,this.setting); this.goldenTime=new GoldenTimeSystem(this.rng,this.setting); this.revenge=new RevengeChanceSystem(); this.lupinBonus=new LupinBonusSystem(this.rng); this.lcdChance={last:null,totalHits:0,weakHits:0,middleHits:0,strongHits:0,normalHits:0,wantedHits:0,wonHits:0,profile:LCD_CHANCE_PROFILE}; this.gameNo=0; this.phase='WAIT_BET'; this.lastRole=null; this.pendingRole=null; this.creditBeforeGame=null; }
  ...
  processNormalLcdChance(){if(this.normal.mode!=='NORMAL')return null;const hit=rollLcdChance('NORMAL',this.rng);if(!hit)return null;this.recordLcdChance(hit);if(!hit.won)return hit;
    const source=`NORMAL_LCD_${hit.key}_VERIFIED_ROUTE`;
    if(hit.destination==='LB_OR_GT'){
      this.normal.pendingReward={type:'LB_OR_GT',source,guarantee:'LB_OR_GT',status:'PENDING_DESTINATION_UNVERIFIED_LB_GT_SPLIT'};
      this.normal.transitionSource=source;this.normal.lastEvent=`${source}_LB_OR_GT_PENDING`;
    }else if(hit.destination==='FUJIKO_ZONE'||hit.destination==='DOROBO_ZONE'){
      this.normal.startCz(hit.destination,source);this.normal.transitionSource=source;this.normal.lastEvent=`${source}_${hit.destination}_AUTO`;
    }
    return hit;
  }
  stopReel(index){if(this.phase!=='SPINNING')return null;const symbol=this.reels.stop(index);if(symbol==null)return null;if(!this.reels.allStopped)return{complete:false,symbol};this.phase='RESULT';this.creditSystem.settle(this.pendingRole);let normal=this.normal.snapshot(),gt=this.goldenTime.snapshot(),revenge=this.revenge.snapshot(),lupinBonus=this.lupinBonus.snapshot(),mode,event;
    if(this.revenge.state==='ACTIVE'){
      ...
      if(lupinBonus.state==='SUCCESS_ART_PENDING_GT'){
        this.goldenTime.reset();this.goldenTime.start({guaranteedStocks:0,source:'LUPIN_BONUS_WIN_STEP6V'});gt=this.goldenTime.snapshot();this.lupinBonus.reset();lupinBonus=this.lupinBonus.snapshot();mode='GOLDEN_TIME';event='LUPIN_BONUS_WIN_GOLDEN_TIME_AUTO_START';
  ...
  startGoldenTimeFromPending(){if(!this.baseNormalReady())return false;const p=this.normal.pendingReward;if(!p||!['GOLDEN_TIME','GOLDEN_TIME_STOCKS'].includes(p.type))return false;const stocks=p.type==='GOLDEN_TIME_STOCKS'?(Number(p.minStocks)||0):0;const ok=this.goldenTime.start({guaranteedStocks:stocks,source:`PENDING_${p.source}`});if(ok){p.status='ROUTED_TO_GOLDEN_TIME_STEP6V';}return ok;}
  ...
  forceLupinBonusEarlyArtForTest(trigger='BATTLE_WIN'){if(this.phase!=='WAIT_BET'||!this.lupinBonus.forceVerifiedEarlyBattleWinForTest(trigger))return false;this.goldenTime.reset();this.goldenTime.start({guaranteedStocks:0,source:`LUPIN_BONUS_EARLY_${trigger}_STEP6V`});this.lupinBonus.reset();return true;}
  snapshot(){return{gameNo:this.gameNo,setting:this.setting,phase:this.phase,role:this.lastRole?.name??'----',normal:this.normal.snapshot(),lcdChance:this.lcdChanceSnapshot(),goldenTime:this.goldenTime.snapshot(),revenge:this.revenge.snapshot(),lupinBonus:this.lupinBonus.snapshot(),reels:this.reels.snapshot(),...this.creditSystem.snapshot()};}
}
