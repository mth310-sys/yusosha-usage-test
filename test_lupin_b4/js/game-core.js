...
import { NormalSystem } from './normal.js?v=step6l';
import { GoldenTimeSystem } from './golden-time.js?v=step6l';
...
export class GameCore {
  ...
  bet(){const playableGtStates=['IDLE','LUPIN_RUSH_ACTIVE','ACTIVE_SET','EXTRA_BONUS_ACTIVE','GOLD_RUSH_ACTIVE','BATTLE_ACTIVE'];if(this.phase!=='WAIT_BET'||!playableGtStates.includes(this.goldenTime.state)||!this.creditSystem.maxBet())return false;this.phase='WAIT_LEVER';return true;}
  ...
  startGoldenTimeFromPending(){if(this.phase!=='WAIT_BET'||this.goldenTime.state!=='IDLE')return false;const p=this.normal.pendingReward;if(!p||!['GOLDEN_TIME','GOLDEN_TIME_STOCKS'].includes(p.type))return false;const stocks=p.type==='GOLDEN_TIME_STOCKS'?(Number(p.minStocks)||0):0;const ok=this.goldenTime.start({guaranteedStocks:stocks,source:`PENDING_${p.source}`});if(ok){p.status='ROUTED_TO_GOLDEN_TIME_STEP6L';}return ok;}
