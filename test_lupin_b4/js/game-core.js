...
import { NormalSystem } from './normal.js?v=step5g';
...
export class GameCore {
  ...
  setSetting(setting){if(this.phase!=='WAIT_BET')return false;this.setting=Number(setting);this.profile=getSettingProfile(this.setting);this.normal.setSetting(this.setting);return true;}
  bet(){if(this.phase!=='WAIT_BET'||!this.creditSystem.maxBet())return false;this.phase='WAIT_LEVER';return true;}
  ...
  startRizeForTest(variant='RIZE'){if(this.phase!=='WAIT_BET'||this.normal.mode!=='NORMAL')return false;return this.normal.startRizeZone(variant,'DEBUG_DIRECT_ENTRY');}
  setRizeBackgroundForTest(background){if(this.phase!=='WAIT_BET')return false;return this.normal.setRizeBackgroundForTest(background);}
  resolveRizeForTest(result){if(this.phase!=='WAIT_BET')return false;return this.normal.resolveRizeForTest(result);}
  seekRaiun100ForTest(level='LOW'){if(this.phase!=='WAIT_BET')return false;return this.normal.seekRaiun100ForTest(level);}
  startRaiunModeForTest(variant='RAIUN'){if(this.phase!=='WAIT_BET')return false;return this.normal.startRaiunModeForTest(variant);}
  resolveRaiunForTest(result){if(this.phase!=='WAIT_BET')return false;return this.normal.resolveRaiunForTest(result);}
  startLegendGateForTest(){if(this.phase!=='WAIT_BET')return false;return this.normal.startLegendGateForTest();}
  setLegendGateMedalsForTest(medals){if(this.phase!=='WAIT_BET')return false;return this.normal.setLegendGateMedalsForTest(medals);}
