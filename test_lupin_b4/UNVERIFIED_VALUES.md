...
| Game-count ceiling reset on setting change | CROSS_SOURCE_CONFIRMED | A successful setting change resets the counter to 0G and redraws the verified 499G/999G ceiling using the new setting. A rejected setting change leaves the current ceiling untouched. See `js/ceiling-runtime-patch.js`. |
