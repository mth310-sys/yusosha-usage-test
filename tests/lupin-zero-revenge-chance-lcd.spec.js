...
test('prior B4 Revenge Chance presentation mechanics are preserved without inventing their lotteries', () => {
  ...
  expect(REVENGE_CHANCE_PRESENTATION_REUSE.verifiedMechanics).toEqual(['COLLECT_FOUR_CHARACTERS','TYPEWRITER_REVENGE_PATTERNS']);
  expect(REVENGE_CHANCE_PRESENTATION_REUSE.characterCollectionTarget).toBe(4);
  expect(REVENGE_CHANCE_PRESENTATION_REUSE.characterCollectionDestination).toBe('LUPIN_BONUS');
  expect(REVENGE_CHANCE_PRESENTATION_REUSE.directGoldenTimeRouteExists).toBe(true);
  expect(REVENGE_CHANCE_PRESENTATION_REUSE.directGoldenTimeRouteTrigger).toBeNull();
  expect(REVENGE_CHANCE_PRESENTATION_REUSE.directGoldenTimeRoutePercent).toBeNull();
  ...
  expect(REVENGE_CHANCE_PRESENTATION_REUSE.automaticSuccessMechanismSelection).toBe(false);
});
...
test('Revenge Chance LCD exposes verified structure but keeps live collection progress unresolved', async ({ page }) => {
  ...
  const state = await page.evaluate(() => {
    ...
    app.core.emit('revenge-chance-enter', { games: 10, source: 'GT_BATTLE', evidenceStatus: 'PUBLISHED_ANALYSIS' });
    return { lcd: app.getRevengeChanceLcdState(), policy: app.revengeChanceLcdPolicy };
  });
  ...
  expect(state.policy.characterCollectionDestination).toBe('LUPIN_BONUS');
  expect(state.policy.directGoldenTimeRouteExists).toBe(true);
  expect(state.policy.directGoldenTimeRouteTrigger).toBeNull();
  expect(state.policy.directGoldenTimeRoutePercent).toBeNull();
...
test('Revenge Chance success LCD does not convert unknown success mechanism into a fake destination split', async ({ page }) => {
  await page.goto('/test_lupin_zero/');
  await page.waitForLoadState('networkidle');
  const state = await page.evaluate(() => {
    const app = window.__LUPIN_ZERO__;
    app.core.emit('revenge-chance-success', { destination: null, destinationCandidates: ['LUPIN_BONUS','GOLDEN_TIME'], destinationSplit: null, source: 'GT_BATTLE', evidenceStatus: 'UNRESOLVED' });
  ...
  expect(state.characterCollectionDestination).toBe('LUPIN_BONUS');
  expect(state.directGoldenTimeRouteExists).toBe(true);
