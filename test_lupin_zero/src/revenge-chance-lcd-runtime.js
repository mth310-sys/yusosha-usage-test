...
function ensureOverlay() {
  ...
  const title = scene.add.text(width / 2, 92, 'REVENGE CHANCE', {
    fontFamily: 'Arial Black, sans-serif',
    fontSize: '18px',
    color: '#f2f2f2',
    stroke: '#000000',
    strokeThickness: 4
  }).setOrigin(.5).setDepth(43).setVisible(false);
  const remaining = scene.add.text(width / 2, 119, '', {
    fontFamily: 'Arial Black, sans-serif',
    fontSize: '13px',
    color: '#ffde73',
    stroke: '#000000',
  ...
  const mechanic = scene.add.text(width / 2, 160, '', {
    ...
    strokeThickness: 2,
    align: 'center',
    wordWrap: { width: Math.max(140, width - 40) }
  }).setOrigin(.5).setDepth(43).setVisible(false);
  const status = scene.add.text(width / 2, 205, '', {
    fontFamily: 'Arial Black, sans-serif',
    fontSize: '12px',
    color: '#ffffff',
    stroke: '#000000',
...
function draw({ remaining = 10, status = 'ACTIVE', destinationCandidates = null } = {}) {
  ...
  o.mechanic.setText('4人のキャラシンボル収集で復活\nタイプライタ復活パターンあり\n※収集発生タイミング・振り分けは未解決').setVisible(true);
  const destinationText = Array.isArray(destinationCandidates) && destinationCandidates.length
    ? `SUCCESS → ${destinationCandidates.join(' / ')}（振り分け未解決）`
    : 'CHARACTER COLLECTION — 実進行は解析待ち';
  o.status.setText(status === 'SUCCESS' ? destinationText : status === 'FAILED' ? 'REVENGE CHANCE END' : destinationText).setVisible(true);
  lastState = Object.freeze({
    visible: true,
    remaining: Math.max(0, Number(remaining) || 0),
    status,
    collectedCharacters: null,
    characterCollectionTarget: REVENGE_CHANCE_LCD_POLICY.characterCollectionTarget,
    typewriterPattern: null,
    destination: null,
    destinationCandidates: Array.isArray(destinationCandidates) ? Object.freeze([...destinationCandidates]) : null
  });
  return lastState;
}
...
core.addEventListener('revenge-chance-enter', (event) => {
  draw({ remaining: event.detail.games ?? 10, status: 'ACTIVE' });
});

core.addEventListener('revenge-chance-game-advanced', (event) => {
  draw({ remaining: event.detail.remaining ?? 0, status: 'ACTIVE' });
});

core.addEventListener('revenge-chance-success', (event) => {
  draw({
    remaining: 0,
    status: 'SUCCESS',
    destinationCandidates: event.detail.destinationCandidates ?? null
  });
});

core.addEventListener('revenge-chance-failed', () => {
  draw({ remaining: 0, status: 'FAILED' });
});

core.addEventListener('mode-exit', (event) => {
  if (event.detail.from === 'REVENGE_CHANCE') hide();
});
core.addEventListener('mode-enter', (event) => {
  if (event.detail.mode !== 'REVENGE_CHANCE' && core.snapshot().mode !== 'REVENGE_CHANCE') hide();
});
