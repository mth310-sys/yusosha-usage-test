// Step 6Z scenario 6/7: use the existing ART-return implementation with a one-shot deterministic test RNG only for the return draw.
export function installGtReturnHitScenarioDebug({core,onChange=()=>{}}={}){
  ...
  if(!panel){panel=document.createElement('section');panel.className='panel';panel.id='gtReturnHitScenarioDebugPanel';panel.innerHTML='<h2>SCENARIO 6/7 / GT RETURN HIT → LB NOTICE</h2><p class="note">既存ART RETURN処理をそのまま通し、引き戻し抽選の1回だけテスト用RNGで当選確定。通常RNGは直後に復元する。</p><div class="panel-head"><button id="gtReturnHitScenarioSetup" type="button">SETUP GT RETURN HIT</button></div><pre id="gtReturnHitScenarioState">NOT RUN</pre>';anchor.parentNode.insertBefore(panel,anchor.nextSibling);}
  ...
  button?.addEventListener('click',()=>{if(!ready())return;const gt=core.goldenTime;gt.start({source:'SCENARIO_6_GT_RETURN_HIT'});gt.state='BATTLE_ACTIVE';gt.result='UNRESOLVED';gt.battleContinuationPct=0;gt.battleResult='HIDDEN';gt.battleSource='SCENARIO_VERIFIED_RETURN_HIT_BOUNDARY';gt.battleGameCount=3;gt.battlePhase='ATTACK_3';gt.battleHiddenOutcome='LOSE';gt.treasurePoints=100000;const normalRng=gt.rng;gt.rng={next(){gt.rng=normalRng;return 0;}};gt.lastEvent='SCENARIO_6_GT_BATTLE_G4_RETURN_HIT_READY';render();onChange();});
